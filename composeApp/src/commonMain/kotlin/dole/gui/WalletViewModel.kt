...
class WalletViewModel(
    ...
    var availableAccounts by mutableStateOf(settingsService.accounts); private set
    ...
    var unsyncedPendingSum: Long by mutableLongStateOf(0); private set
    ...
    fun attemptLogin(pin: String) {
        ...
        if (acc.pinHash.isNotEmpty()) {
            val inputHash = CryptoUtils.bytesToHex(CryptoUtils.sha256(pin.encodeToByteArray()))
            ...
            if (c != null) {
                ...
                if (matchesPhysical || acc.pinHash.isEmpty()) {
                    try {
                        ...
                        val retries = try { c.pinRetries } catch(_:Exception) { 3 }
                        if (retries == 0) c.disconnect()
                        else if (c.verifyPin(pin.encodeToByteArray())) isPinCorrectOnCard = true
                        else c.disconnect()
                    } catch (_: Exception) {
                        try {
                            c.disconnect()
            ...
            withContext(Dispatchers.Main) {
                if (allowLogin) {
                    if (acc.pinHash.isEmpty() && isPinCorrectOnCard) {
                        val newHash = CryptoUtils.bytesToHex(CryptoUtils.sha256(pin.encodeToByteArray()))
    ...
    fun showUserMessage(msg: String) {
        ...
        viewModelScope.launch {
            delay(3000)
            if (userMessage == msg) userMessage = null
        }
    }
            ...
            try {
                ...
                while (isActive) {
                    try {
                        ...
                        if (currentCard.isConnected) {
                            val id = CryptoUtils.getPersonIdAsHex(currentCard.publicKey)
                ...
                val pinB = pin.encodeToByteArray()
                try {
                    if (newCardHasPin) {
                ...
                val id = CryptoUtils.getPersonIdAsHex(c.publicKey)
    ...
    fun changeCardPin(newPin: String, onSuccess: () -> Unit, onError: () -> Unit) {
        ...
        settingsJob = viewModelScope.launch(Dispatchers.IO) {
            try {
                ...
                if (!isActive) return@launch
                val activeCard = c!!
                val newPinBytes = newPin.encodeToByteArray()
    ...
    private fun connectWalletService(card: SmartCard, pin: String) {
        ...
        viewModelScope.launch(Dispatchers.IO) {
            try {
                ...
                val realCardId = try {
                    CryptoUtils.getPersonIdAsHex(card.publicKey)
                } catch (_: Exception) {
                ...
                ws.setOnStateUpdateListener { state ->
                    viewModelScope.launch(Dispatchers.Main) {
                        ...
                        _unsyncedTransactions = state.unsyncedIncoming.map { tx ->
                            val amount = when (tx) {
                                is SendTransaction -> tx.goc.toInt()
                                is MintTransaction -> tx.goc.toInt()
                                is BurnTransaction -> tx.goc.toInt()
                                else -> 0
                            }
                            DisplayTransaction(tx, amount, true)
                        }
        ...
        balanceResult.items.forEach { d ->
            ...
            val sign =
                when (tx) {
                    ...
                    is SendTransaction -> if (tx.target == uid) 1 else -1
                    else -> -1
                }
    ...
    private fun calculateBalanceFromSnapshots(
        ...
        for (v in received.values) totalReceived += v.toLong()

        var totalSent = 0L
        for (v in sent.values) totalSent += v.toLong()
    ...
    private fun savePendingToDisk() {
        val uid = currentId ?: return
        settingsService.savePendingActions(uid, pendingActions)
    }

    private fun loadPendingFromDisk() {
        val uid = currentId ?: return
        pendingActions = settingsService.loadPendingActions(uid)
    }
    ...
    private suspend fun verifyPinOrAbort(activeCard: SmartCard, pinB: ByteArray): Boolean {
        if (!activeCard.verifyPin(pinB)) {
            val left = try { activeCard.pinRetries } catch (_: Exception) { 0 }
