...
import dole.ledger.AndroidLedgerService
...
class MainActivity : ComponentActivity() {
    ...
    private var ledgerService: AndroidLedgerService? = null
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        try {
            ...
            val settingsService = SettingsService(path)
            val ledger = Ledger()
            settingsService.attachLedger(ledger)

            ledgerService = AndroidLedgerService.getInstance(
                applicationContext, Constants.DITTO_APP_ID, Constants.DITTO_PLAYGROUND_TOKEN
            )
        ...
        setContent {
            if (::viewModel.isInitialized) WalletApp(viewModel)
        }
    }
